    ...
    private void initAliVcPlayer() {
        ...
        mAliyunVodPlayer.setOnCompletionListener(new IAliyunVodPlayer.OnCompletionListener() {
            ...
            public void onCompletion() {
                ...
                //如果当前播放资源是本地资源时, 再显示replay :: 这里改为不管是本地还是网络资源都显示重播
//                if (mTipsView != null && isLocalSource()) {
    ...
    public void setVidSts(String title, String videoID, String tempAccessKeySecret, String tempToken, String tempAccessKeyId) {
        AliyunVidSts vidSts = new AliyunVidSts();
        vidSts.setAcId(tempAccessKeyId);
        vidSts.setAkSceret(tempAccessKeySecret);
        vidSts.setSecurityToken(tempToken);
        vidSts.setVid(videoID);
        vidSts.setTitle(title);
        this.setVidSts(vidSts);
    }
    ...
    private void startProgressUpdateTimer() {
        if (mProgressUpdateTimer != null) {
            ...
            mProgressUpdateTimer.sendEmptyMessage(0);
        }
    }
    ...
    public void setOnShowMoreClickListener(ControlView.OnShowMoreClickListener listener) {
        // 横屏下显示更多
        mControlView.setOnShowMoreClickListener(listener);
    }

    public void setOnShowControlListener(ControlView.IVisibilityListener listener) {
        mControlView.setIVisibilityListener(listener);
    }

    public void hideTitle() {
        if (mControlView != null) {
            mControlView.hideTitleView();
        }
    }
}
